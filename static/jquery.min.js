			...
			n = {
				...
				setup: function () {
					...
					0 < b.fixedColumns &&
						...
						(a.wrap('<div class="fht-fixed-body"></div>'), c('<div class="fht-fixed-column"></div>').prependTo(f), (k = f.find('.fht-fixed-body')));
					...
					a.hasClass('fht-table-init')
						...
						: ((k =
								0 < b.fixedColumns
									? c('<div class="fht-thead"><table class="fht-table"></table></div>').prependTo(k)
									: c('<div class="fht-thead"><table class="fht-table"></table></div>').prependTo(f)),
			...
			h = {
				...
				_setupFixedColumn: function (a, d, e) {
					...
					d.find('.fht-tbody .fht-table').bind('mousewheel', function (a, d, b, e) {
						if (0 != e)
							return (
								(a = parseInt(c(this).css('marginTop'), 10) + (0 < e ? 120 : -120)), 0 < a && (a = 0), a < -n && (a = -n), c(this).css('marginTop', a), f.find('.fht-tbody').scrollTop(-a).scroll(), !1
							);
					});
					f.css({ width: g });
				...
				_setupTableFooter: function (a, d, e) {
					...
					a.length ||
						(a =
							0 < b.fixedColumns
								? c('<div class="fht-tfoot"><table class="fht-table"></table></div>').appendTo(d.find('.fht-fixed-body'))
								: c('<div class="fht-tfoot"><table class="fht-table"></table></div>').appendTo(d));
					a.find('table.fht-table').addClass(b.originalTable.attr('class'));
					switch (!0) {
						...
						c(this)
							...
							.each(function (a) {
								...
								if ('undefined' === typeof d[a] || 1 != d[a].colspan)
									(d[a] = { width: c(this).width() + parseInt(c(this).css('border-left-width')) + parseInt(c(this).css('border-right-width')), colspan: f }), 1 == f && g++;
							});
						if (g == f) return !1;
					});
					return d;
						...
						c(this)
							...
							.each(function (a) {
								...
								'undefined' !== typeof d[a] &&
									d[a].colspan == f &&
									((c(this).find('div.fht-cell').length ? c(this).find('div.fht-cell') : c('<div class="fht-cell"></div>').appendTo(c(this))).css({ width: parseInt(d[a].width, 10) }),
									c(this).closest('.fht-tbody').length ||
										!c(this).is(':last-child') ||
										c(this).closest('.fht-fixed-column').length ||
										((a = Math.max((c(this).innerWidth() - c(this).width()) / 2, b.scrollbarOffset)), c(this).css({ 'padding-right': parseInt(c(this).css('padding-right')) + a + 'px' })));
							});
					});
				},
				...
				_getScrollbarWidth: function () {
					...
					if (!a)
						if (/msie/.test(navigator.userAgent.toLowerCase())) {
							var b = c('<textarea cols="10" rows="2"></textarea>').css({ position: 'absolute', top: -1e3, left: -1e3 }).appendTo('body'),
								e = c('<textarea cols="10" rows="2" style="overflow: hidden;"></textarea>').css({ position: 'absolute', top: -1e3, left: -1e3 }).appendTo('body'),
								a = b.width() - e.width() + 2;
							b.add(e).remove();
						} else
							(b = c('<div />')
								.css({ width: 100, height: 100, overflow: 'auto', position: 'absolute', top: -1e3, left: -1e3 })
								.prependTo('body')
								.append('<div />')
								.find('div')
